...
@Service
@Slf4j
public class PostgreSQLReconciliationService {
                ...
                for (Map<String, Object> order : orders) {
                    try {
                        ...
                        jdbcTemplate.update(
                                ...
                                discrepancyAmount, false, ifCode, backupIfCode);
                ...
                try {
                    ...
                    if (exists) {
                        ...
                        jdbcTemplate.update(
                                ...
                                "discrepancy_amount = ?, is_fixed = FALSE, " +
            ...
            for (String orderId : orderIds) {
                try {
                    ...
                    jdbcTemplate.update(
                            ...
                            "VALUES (?, 0, 'NONE', 0, false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)",
                            orderId);
                    count++;
                } catch (Exception e) {
    ...
    public List<PaymentReconciliation> findAllUnfixedDiscrepancies() {
        try {
            ...
            // 检查表是否存在
            if (!checkTableExists("payment_reconciliation")) {
                log.error("payment_reconciliation表不存在，无法查询差异记录");
                return List.of();
            }
            
            try {
                // 使用更明确的查询方式，避免类型和语法问题
                String sql = 
                    "SELECT " +
                    "  order_no, " +
                    "  expected, " +
                    ...
                    "WHERE discrepancy_type <> 'NONE' AND is_fixed = false";
                
                return jdbcTemplate.query(sql, (rs, rowNum) -> {
                    PaymentReconciliation record = new PaymentReconciliation();
                    record.setOrderNo(rs.getString("order_no"));
                    record.setExpectedAmount(rs.getBigDecimal("expected"));
                    record.setActualAmount(rs.getBigDecimal("actual"));
                    record.setDiscrepancyType(rs.getString("discrepancy_type"));
                    record.setDiscrepancyAmount(rs.getBigDecimal("discrepancy_amount"));
                    record.setIsFixed(rs.getBoolean("is_fixed"));
                    record.setChannel(rs.getString("channel"));
                    record.setBackupIfCode(rs.getString("backup_if_code"));
                    return record;
                });
            } catch (Exception e) {
                ...
                // 尝试使用更简单的查询
                log.info("尝试使用更简单的查询方式...");
                try {
                    String simpleSql = 
                        "SELECT order_no, expected, actual, discrepancy_type, " +
                        "discrepancy_amount, is_fixed, channel, backup_if_code " +
                        "FROM payment_reconciliation " +
                        "WHERE discrepancy_type::text <> 'NONE'::text AND is_fixed = false";
                    ...
                    log.error("简单查询也失败: {}", ex.getMessage(), ex);
                    
                    // 使用最基本的方式查询，不依赖于表结构
                    log.info("尝试使用最基本的查询方式...");
                    try {
                        ...
                        for (String orderNo : orderNos) {
                            ...
                            record.setExpectedAmount(BigDecimal.ZERO);
                            record.setDiscrepancyType("MISSING_PAYMENT");
                            record.setDiscrepancyAmount(BigDecimal.ZERO);
                        ...
                        log.error("所有查询方式都失败: {}", finalEx.getMessage(), finalEx);
                        return List.of();
                    }
                }
            }
    ...
    public PaymentReconciliation findByOrderNo(String orderNo) {
        try {
            // 检查表是否存在
            if (!checkTableExists("payment_reconciliation")) {
                log.error("payment_reconciliation表不存在，无法查询对账记录");
                return null;
            }
            
            String sql = "SELECT * FROM payment_reconciliation WHERE order_no = ?";
            List<PaymentReconciliation> results = jdbcTemplate.query(
                    sql, 
                    new BeanPropertyRowMapper<>(PaymentReconciliation.class),
                    orderNo);
    ...
    public boolean markAsFixed(String orderNo) {
        try {
            // 检查表是否存在
            if (!checkTableExists("payment_reconciliation")) {
                log.error("payment_reconciliation表不存在，无法标记对账差异已修复");
                return false;
            }
            
            String sql = "UPDATE payment_reconciliation SET is_fixed = true, update_time = now() WHERE order_no = ?";
            int updated = jdbcTemplate.update(sql, orderNo);
            return updated > 0;
        } catch (Exception e) {
    ...
    public Map<String, Object> getReconciliationStats() {
        try {
            ...
            String sql = "SELECT " +
                    ...
                    "SUM(CASE WHEN discrepancy_type != 'NONE' AND is_fixed = true THEN 1 ELSE 0 END) AS fixed_discrepancies, " +
    ...
    public List<PaymentReconciliation> findDiscrepanciesByDateRange(String startDate, String endDate) {
        try {
            ...
            // 检查表是否存在
            if (!checkTableExists("payment_reconciliation")) {
                log.error("payment_reconciliation表不存在，无法查询日期范围的差异记录");
                return List.of();
            }
            
            String sql = "SELECT * FROM payment_reconciliation " +
                    "WHERE discrepancy_type != 'NONE' " +
                    "AND create_time BETWEEN ?::timestamp AND (?::timestamp + interval '1 day')";
    ...
    public int fixAllDiscrepancies() {
        try {
            // 检查表是否存在
            if (!checkTableExists("payment_reconciliation")) {
                log.error("payment_reconciliation表不存在，无法修复所有差异");
                return 0;
            }
            
            String sql = "UPDATE payment_reconciliation SET is_fixed = true, update_time = now() " +
                    "WHERE discrepancy_type != 'NONE' AND is_fixed = false";
